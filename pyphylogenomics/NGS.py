...
def separate_by_index(fastq_file, index_list, folder="", levenshtein_distance=1):
    ...
    * ``levenshtein_distance`` *Optional*, default = 1: Maximum number of different nucleotides that will be accepted when comparing template and sequenced indexes (due to errors in base calling during sequencing).
...
# ----------------------------------------------------------------------------
# @input: output from running velvet assembly on all Kmer values
# @output: a dictionary with the parameters: kmer, nodes, n50, max, total
def get_velvet_params(output):
    '''
    \* *Internal function* \*
